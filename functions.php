<?php
/**
 * blogify defining constants, adding files and WordPress core functionality.
 *
 */
...
if ( ! function_exists( 'blogify_setup' ) ):
	 ...
	 */
	add_action( 'after_setup_theme', 'blogify_setup' );
	...
	function blogify_setup() {
		/**
		 * blogify_add_files hook
		 *
		...
		/**
		 * blogify_add_functionality hook
		 *
		 * Adding other addtional functionality if needed.
		 */
		do_action( 'blogify_add_functionality' );
		...
		register_nav_menu( 'primary', __( 'Primary Menu', 'blogify' ) );

		// Add blogify custom image sizes
		 ...
		 * hard-coded <#444title> tag in the document head, and expect WordPress to
		 * provide it for us.
		 */
