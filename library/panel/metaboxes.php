 ...
 */
 ...
 add_action( 'add_meta_boxes', 'blogify_add_custom_box' );
/**
 * Add Meta Boxes.
 *
 * Add Meta box in page and post post types.
 */
function blogify_add_custom_box() {
	add_meta_box(
		'siderbar-layout',							  										//Unique ID
		__( 'Select layout for this specific Page only ( Note: This setting only reflects if page Template is set as Default Template and Blog Type Templates.)', 'blogify' ),   	//Title
		'blogify_sidebar_layout',                   							//Callback function
		'page'                                          							//show metabox in pages
	);
	add_meta_box(
		'siderbar-layout',							  										//Unique ID
		__( 'Select layout for this specific Post only', 'blogify' ),   	//Title
		'blogify_sidebar_layout',                   							//Callback function
...
$sidebar_layout = array(
							'default-sidebar' 		=> array(
															'id'        => 'blogify_sidebarlayout',
															'value'     => 'default',
															'label'     => __( 'Default Layout Set in', 'blogify' ).' '.'<a href="'.esc_url( home_url( '/' ) ).'/wp-admin/themes.php?page=theme_options" target="_blank">'.__( 'Theme Settings', 'blogify' ).'</a>',
															'thumbnail' => ' '
															),
							'no-sidebar'         => array(
															'id'        => 'blogify_sidebarlayout',
															'value'     => 'no-sidebar',
															'label'     => __( 'No sidebar', 'blogify' ),
															'thumbnail' => get_template_directory_uri() . '/library/panel/images/no-sidebar.png'
															),
							'no-sidebar-full-width' => array(
															'id'        => 'blogify_sidebarlayout',
															'value'     => 'no-sidebar-full-width',
															'label'     => __( 'No sidebar, Full Width', 'blogify' ),
 ...
 */
function blogify_sidebar_layout() {
...
add_action('save_post', 'blogify_save_custom_meta');
/**
 * save the custom metabox data
 * @hooked to save_post hook
 */
function blogify_save_custom_meta( $post_id ) {
	global $sidebar_layout, $post;
